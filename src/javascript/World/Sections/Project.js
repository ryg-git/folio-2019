...
// import { draw } from './my-confetti-1.js'
...
export default class Project {
    ...
    getTextGeometryScene() {
        const txtgeometry = new THREE.TextGeometry( this.name, {
        ...
        firstMesh.name = `ydn_${this.name}_${Date.now()}`;
    ...
    setFloor() {
        ...
        this.floor.area = this.areas.add({
            position: new THREE.Vector2(
                this.x + this.link.x + 4,
                this.y + this.floor.y + this.link.y - 1
            ),
        ...
        this.floor.areaLabel.position.x = this.link.x + 4;
        this.floor.areaLabel.position.y = this.link.y - 1;
        ...
        const txtScene = this.getTextGeometryScene();

        const board = {};
        board.x = this.x + this.boards.xStart + this.boards.xInter;
        board.y = this.y + this.boards.y;
        ...
        this.objects.add({
            ...
            offset: new THREE.Vector3(board.x - 2.5, board.y, 5),
            rotation: new THREE.Euler(0, 0, 0),
            duplicated: true,
            mass: 0
        })
        ...
        console.log(this.resources.items.cupcakeBase);

        // const s = new THREE.Scene();

        // s.add(this.resources.items.cupcakeBase.scene.children[0])
        
        this.objects.addObject3D(
            this.resources.items.cupcakeBase,
        )
    }
}
